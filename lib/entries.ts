import { Entry } from "./types";

export const entries: Entry[] = [
	{
		date: "2024-10-01",
		title: "Arona",
		description: ["All Blue, that's why it's called Blue Archive, sensei!"],
		note: [
			"I started this Inktober trend back in 2018, but at that time I only drew stuff based on my mood (or based on inktober's official prompt -- that's usually so ambiguous that I don't even know what to draw).",
			"So, started in 2024, to give sense of direction, I'm applying some sort of theme. A Blue Archive theme.",
		],
		school: "S.C.H.A.L.E",
		characters: [
			{
				name: "Arona",
				club: "S.C.H.A.L.E",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-01.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utnycxdq2q",
		twitter: "https://x.com/Victim_Crasher/status/1841121005919748146",
	},
	{
		date: "2024-10-02",
		title: "Plana",
		description: ["When the rain comes... again"],
		note: [
			"I'm thinking to follow the character appearance (in the inktober) just like how they were introduced in the story (Chapter 1, 2, ...). But there were times to make an exception so the character doesn't feel out of place. This day's submission is one of them.",
		],
		school: "S.C.H.A.L.E",
		characters: [
			{
				name: "Plana",
				club: "S.C.H.A.L.E",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-02.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utpemmla2q",
		twitter: "https://x.com/Victim_Crasher/status/1841474437583077417",
	},
	{
		date: "2024-10-03",
		title: "Rin & President",
		description: ["The time it once was"],
		note: [
			"Until the time of the writing, GSC President whereabout is still unknown. But I really like the dynamic between her and Rin.",
		],
		school: "General Student Council",
		characters: [
			{
				name: "GSC President",
				club: "General Student Council",
			},
			{
				name: "Rin",
				club: "General Student Council",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-03.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utqp2oli2q",
		twitter: "https://x.com/Victim_Crasher/status/1841850125397946591",
	},
	{
		date: "2024-10-04",
		title: "First Encounter",
		description: ["The beginning of it all."],
		note: [
			"So I actually drew this before Volume 1 Chapter 4 came up and with big speculation on how the things happened in the first place.",
			"It's not far off though, so I'm happy.",
		],
		school: "Abydos",
		characters: [
			{
				name: "Takanashi Hoshino",
				club: "Forceclosure Task Force",
			},
			{
				name: "Kuchinashi Yume",
				club: "Abydos High School's Student Council",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-04.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utrqspix2v",
		twitter: "https://x.com/Victim_Crasher/status/1842202984341799057",
	},
	{
		date: "2024-10-05",
		title: "The Heist",
		description: [
			'"Nn. let\'s run now." - No. 2',
			'"Hehe it\'s fun~" - No.3',
			'"Hey you! get back here!" - Bank clerk',
		],
		note: ['Trying a "dynamic" pose here, goes pretty nice actually.'],
		school: "Abydos",
		characters: [
			{
				name: "Sunaookami Shiroko",
				club: "Forceclosure Task Force",
			},
			{
				name: "Izayoi Nonomi",
				club: "Forceclosure Task Force",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-05.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utstefm42u",
		twitter: "https://x.com/Victim_Crasher/status/1842575507445149958",
	},
	{
		date: "2024-10-06",
		title: "The Heist Part 2",
		description: ['"Wait!!! Wait for me!!!!!!" - No. 4', '"Hahaha, well all for the mission, I guess" - No. 0'],
		note: ["Some inktober submission was related, so please look forward for it in the upcoming pages! ;)"],
		school: "Abydos",
		characters: [
			{
				name: "Kuromi Serika",
				club: "Forceclosure Task Force",
			},
			{
				name: "Okusora Ayane",
				club: "Forceclosure Task Force",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-06.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uttox7ml2j",
		twitter: "https://x.com/Victim_Crasher/status/1842929526571241885",
	},
	{
		date: "2024-10-07",
		title: "Faust",
		description: ["One and only, the head of the Masked Swimsuit Gang. The notorious Faust."],
		note: [
			"Hehe taiyaki bag.",
			"As a FYI, go-home club is assigned for students that has no club affiliation. But, when I first saw the title, I thought it was like, a real club. lmao.",
		],
		school: "Trinity",
		characters: [
			{
				name: "Ajitani Hifumi",
				club: "Make-Up Work Club",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-07.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utujxisy2l",
		twitter: "https://x.com/Victim_Crasher/status/1843286464274239581",
	},
	{
		date: "2024-10-08",
		title: "Scammed",
		description: ['"This... this is just a mug with a mug print in it!?"'],
		note: [
			"There was meme circulating in the internet of this scam listing, so please read the reviews first, guys.",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Rikuhachima Aru",
				club: "Problem Solver 68",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-08.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utvgkepf2e",
		twitter: "https://x.com/Victim_Crasher/status/1843656155052351701",
	},
	{
		date: "2024-10-09",
		title: "Tuning In",
		description: ["\"I wonder what's the commotion?\" - Kayoko, probably wondering what's the fuss about"],
		note: ["The earphone cable was too short lmao.", "(Part 2 of PS68 story)"],
		school: "Gehenna",
		characters: [
			{
				name: "Onikata Kayoko",
				club: "Problem Solver 68",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-09.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utwahwp52y",
		twitter: "https://x.com/Victim_Crasher/status/1844028045688238564",
	},
	{
		date: "2024-10-10",
		title: "The Returned Package",
		description: [
			'"Hehe, good morning Mr. Scam Seller, we\'d like to return a package"',
			'"A week later somehow all of those scam product listing were gone from the marketplace."',
		],
		note: ['The text in the bomb is "Armed" not "Rameo"...', "(Final Part of PS68 story)"],
		school: "Gehenna",
		characters: [
			{
				name: "Igusa Haruka",
				club: "Problem Solver 68",
			},
			{
				name: "Asagi Mutsuki",
				club: "Problem Solver 68",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-10.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utwzulif2e",
		twitter: "https://x.com/Victim_Crasher/status/1844392333397328192",
	},
	{
		date: "2024-10-11",
		title: "Seconds From Disaster",
		description: ["Bro why both of them look so mad brooo?"],
		note: ["This was based from a meme."],
		school: "Gehenna",
		characters: [
			{
				name: "Amau Ako",
				club: "Prefect Team",
			},
			{
				name: "Sorasaki Hina",
				club: "Prefect Team",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-11.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utym35qf2y",
		twitter: "https://x.com/Victim_Crasher/status/1844752234858586439",
	},
	{
		date: "2024-10-12",
		title: "Trap",
		description: ["Don't ask me why Iori is barefooted."],
		note: [
			"So for dark skinned students, I actually tried to shade them using a pencil, sometimes it looks good, sometimes it's kinda meh lmao.",
			"Also feet practice.",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Hinomiya Chinatsu",
				club: "Problem Solver 68",
			},
			{
				name: "Shiromi Iori",
				club: "Problem Solver 68",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-12.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6utzmho4l2j",
		twitter: "https://x.com/Victim_Crasher/status/1845102186394689735",
	},
	{
		date: "2024-10-13",
		title: "Gamer's Rage",
		description: ["Ni-", "Nah, canon Momoi is a sweet girl, guys."],
		note: ["Drawing Saiba twins in black and white is a bit challenging, but I hope you know which is which."],
		school: "Millennium",
		characters: [
			{
				name: "Saiba Momoi",
				club: "Game Development Department",
			},
			{
				name: "Saiba Midori",
				club: "Game Development Department",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-13.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uu2h6b4q2q",
		twitter: "https://x.com/Victim_Crasher/status/1845487687991705908",
	},
	{
		date: "2024-10-14",
		title: "Prank",
		description: ["5head tic-tac-toe game."],
		note: [
			'TBH, I really like how this turns out, everything looks "correct" and "nice".',
			"Also funny enough a few days before I made this artbook. I saw a meme about this tic-tac-toe on Yuzu's forehead. So, I guess I was ahead by a year xD",
		],
		school: "Millennium",
		characters: [
			{
				name: "Tendou Alice",
				club: "Game Development Department",
			},
			{
				name: "Hanaoka Yuzu",
				club: "Game Development Department",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-14.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uu3gaknn2e",
		twitter: "https://x.com/Victim_Crasher/status/1845837295431360699",
	},
	{
		date: "2024-10-15",
		title: "Ikazuchi-chan",
		description: [
			"It's just a simple maintenance, please don't add additional \"functions\", okay Utaha?",
			'*proceeds to add 100 miscellaneous "features" to Ikazuchi*',
		],
		note: [
			"Ikazuchi-chan is the name of Utaha's turret.",
			"Interesting thing is, Utaha fanbase is very solid, the amount of people liking this particular artwork is actually higher than average. So shout out to Utaha fans!",
		],
		school: "Millennium",
		characters: [
			{
				name: "Shiraishi Utaha",
				club: "Engineering Department",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-15.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uu4rhovi2l",
		twitter: "https://x.com/Victim_Crasher/status/1846199133197291672",
	},
	{
		date: "2024-10-16",
		title: "Pairing",
		description: [
			"So you see, if we add one more feature to Ikazuchi-chan I think wecan win this year's Millenium Awards!",
			"Well it's worth a shot.",
		],
		note: ["Hibiki is certainly has an interesting character design."],
		school: "Millennium",
		characters: [
			{
				name: "Nekozuka Hibiki",
				club: "Engineering Department",
			},
			{
				name: "Toyomi Kotori",
				club: "Engineering Department",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-16.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uu5rykb72v",
		twitter: "https://x.com/Victim_Crasher/status/1846576618959192439",
	},
	{
		date: "2024-10-17",
		title: "Frenzy Max",
		description: [
			"I'm coming with our new stocks of fre- AHHHHHHHHH",
			"Afterwards, they spent the entire day cleaning the mess they created.",
		],
		note: [
			"Frenzy Max is an energy drink brand based on m*nster. The design ingame has actually been altered to not resembles the brand too much (it used to look almost the same as the real world m*nster).",
		],
		school: "Millennium",
		characters: [
			{
				name: "Otose Kotama",
				club: "Veritas",
			},
			{
				name: "Omagari Hare",
				club: "Veritas",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-17.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uu6rvjjb2l",
		twitter: "https://x.com/Victim_Crasher/status/1846922013480571303",
	},
	{
		date: "2024-10-18",
		title: "Hitchhiking",
		description: ["Just stay put and hang tight senpai, we'll arrive to our destination in no time!"],
		note: ["Drawing binah is quite a challenge...."],
		school: "Millennium",
		characters: [
			{
				name: "Binah",
				club: undefined,
			},
			{
				name: "Konuri Maki",
				club: "Veritas",
			},
			{
				name: "Kagami Chihiro",
				club: "Veritas",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-18.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uu7edyca2q",
		twitter: "https://x.com/Victim_Crasher/status/1847285554695852243",
	},
	{
		date: "2024-10-19",
		title: "Bubble Tea",
		description: ["Hi sensei! Ah this? it's the new bubble tea that's just opened in Millenium!"],
		note: ["Bubble tea challenge hehe..."],
		school: "Millennium",
		characters: [
			{
				name: "Ichinose Asuna",
				club: "Cleaning&Clearing",
			},
			{
				name: "Kakudate Karin",
				club: "Cleaning&Clearing",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-19.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6uucasn4s2r",
		twitter: "https://x.com/Victim_Crasher/status/1847653729383989502",
	},
	{
		date: "2024-10-20",
		title: "Sukajan",
		description: ["The Sukajan stays on whatever happens!"],
		note: [
			"Drawing Neru's sukajan is like, so complex....",
			"Also drawing Neru made me realized that her ahoge was a bit, interesting, to say the least.",
		],
		school: "Millennium",
		characters: [
			{
				name: "Murokasa Akane",
				club: "Cleaning&Clearing",
			},
			{
				name: "Mikamo Neru",
				club: "Cleaning&Clearing",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-20.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6xagmt5zp2e",
		twitter: "https://x.com/Victim_Crasher/status/1848011863470297391",
	},
	{
		date: "2024-10-21",
		title: "Confidential",
		description: ["Some things are better left unread."],
		note: [
			"Challengine myself to draw 3 students in one inktober submission. Kinda hard to make each students separated, so that's why I added thicker lines around Koyuki.",
		],
		school: "Millennium",
		characters: [
			{
				name: "Ushio Noa",
				club: "Seminar",
			},
			{
				name: "Kurosaki Koyuki",
				club: "Seminar",
			},
			{
				name: "Hayase Yuuka",
				club: "Seminar",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-21.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l6zq2eq2mw2l",
		twitter: "https://x.com/Victim_Crasher/status/1848371019221962955",
	},
	{
		date: "2024-10-12",
		title: "Abi-Eshuh",
		description: ["Doing a regular checking, making sure everything is working as expected."],
		note: ["Drawing Abi-Eshuh is very challenging, but very rewarding!"],
		school: "Millennium",
		characters: [
			{
				name: "Asuma Toki",
				club: "Cleaning&Clearing, Paranormal Affairs Department",
			},
			{
				name: "Tsukatsuki Rio",
				club: "Seminar",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-22.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l74dr75zng2y",
		twitter: "https://x.com/Victim_Crasher/status/1848748066092073037",
	},
	{
		date: "2024-10-23",
		title: "Thermostat",
		description: ["The never ending battle of the thermostat..."],
		note: ["Super happy with how this turns out TBH."],
		school: "Millennium",
		characters: [
			{
				name: "Akeboshi Himari",
				club: "Paranormal Affairs Department",
			},
			{
				name: "Izumimoto Eimi",
				club: "Paranormal Affairs Department",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-23.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l76wrfwlhc2z",
		twitter: "https://x.com/Victim_Crasher/status/1849121633472278580",
	},
	{
		date: "2024-10-24",
		title: "Azoos",
		description: ["ᓀ‸ᓂ"],
		note: ["We now moved to Trinity, and starting our way up to Volume 3."],
		school: "Trinity",
		characters: [
			{
				name: "Shirasu Azusa",
				club: "Make-Up Work Club",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-24.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7bavunzid2z",
		twitter: "https://x.com/Victim_Crasher/status/1849455907593478395",
	},
	{
		date: "2024-10-25",
		title: "The Body?!",
		description: ["The face huh?", "The body?", "The body??", "The bodyyyyyy!!!!!"],
		note: ["Based on that scene from Mr. Bean."],
		school: "Trinity",
		characters: [
			{
				name: "Urawa Hanako",
				club: "Make-Up Work Club",
			},
			{
				name: "Shimoe Koharu",
				club: "Make-Up Work Club, Justice Task Force",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-25.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7dqljflky2y",
		twitter: "https://x.com/Victim_Crasher/status/1849814394374586702",
	},
	{
		date: "2024-10-26",
		title: "Roll Cake",
		description: ["Calm down Mika, it's just a drawing!"],
		note: ["Based on that scene from Spongebob Squarepants."],
		school: "Trinity",
		characters: [
			{
				name: "Kirifuji Nagisa",
				club: "Tea Party",
			},
			{
				name: "Misono Mika",
				club: "Tea Party",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-26.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7gk7xiv2m2l",
		twitter: "https://x.com/Victim_Crasher/status/1850218405816946996",
	},
	{
		date: "2024-10-27",
		title: "Cramming",
		description: ["The probable canon explanation (aside from recovery) on why she's not playable yet"],
		note: [
			"At the time of that drawing, Seia is not yet released.",
			"It is now though. But I still think that's probably why she took so long to be playable (story wise).",
		],
		school: "Trinity",
		characters: [
			{
				name: "Yurizono Seia",
				club: "Tea Party",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-27.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7ix75sa4p2l",
		twitter: "https://x.com/Victim_Crasher/status/1850564891528274359",
	},
	{
		date: "2024-10-28",
		title: "Cake",
		description: ["Tsurugi wanted to say something but I think it's better to just let this slide."],
		note: ["Hasumi's garter belt... I can't really fathom drawing it, so that's why I was cheating a bit."],
		school: "Trinity",
		characters: [
			{
				name: "Kenzaki Tsurugi",
				club: "Justice Task Force",
			},
			{
				name: "Hanekawa Hasumi",
				club: "Justice Task Force",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-28.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7ldp6ojwc2u",
		twitter: "https://x.com/Victim_Crasher/status/1850909422551499128",
	},
	{
		date: "2024-10-29",
		title: "Splitting Image",
		description: ["People with Blue Eyes be like"],
		note: ["Shout out to Kelsuis and their Amiyi art.", " ", " ", " ", " ", "Kelsuis orang mana sih?"],
		school: "Trinity",
		characters: [
			{
				name: "Nakamasa Ichika",
				club: "Justice Task Force",
			},
			{
				name: "Shizuyama Mashiro",
				club: "Justice Task Force",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-29.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7nuh5gyc52v",
		twitter: "https://x.com/Victim_Crasher/status/1851272735802212757",
	},
	{
		date: "2024-10-30",
		title: "Cleaning Day",
		description: ["Not so wappi~ day unfortunately."],
		note: ["Sakurako is that one character that really grow on me, protecc."],
		school: "Trinity",
		characters: [
			{
				name: "Iochi Mari",
				club: "Sisterhood",
			},
			{
				name: "Wakaba Hinata",
				club: "Sisterhood",
			},
			{
				name: "Utazumi Sakurako",
				club: "Sisterhood",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-30.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7qgnrcmek2l",
		twitter: "https://x.com/Victim_Crasher/status/1851642970795397559",
	},
	{
		date: "2024-10-31",
		title: "Thank You",
		description: ["Inktober 2024", "31/31"],
		note: ["The end of Inktober 2024. I didn't expect to be able to finish the full Inktober without skipping."],
		school: "S.C.H.A.L.E",
		characters: [
			{
				name: "Anime Sensei",
				club: "S.C.H.A.L.E",
			},
			{
				name: "GDD Manga Sensei",
				club: "S.C.H.A.L.E",
			},
			{
				name: "Doodle Sensei",
				club: "S.C.H.A.L.E",
			},
			{
				name: "Benriya 68 Manga Sensei",
				club: "S.C.H.A.L.E",
			},
		],
		isLandscape: false,
		image: "/entries/2024-10-31.webp",
		pixiv: "https://www.pixiv.net/en/artworks/128598570",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3l7swuyceq62g",
		twitter: "https://x.com/Victim_Crasher/status/1852004595670876558",
	},
	{
		date: "2025-10-01",
		title: "Year Two!",
		description: ["Year two! Let's goooo!"],
		note: [
			"Second year of Inktober, Blue Archive theme. I was thinking to draw Arona first just to derust my traditional drawing skills.",
		],
		school: "S.C.H.A.L.E",
		characters: [
			{
				name: "Arona",
				club: "S.C.H.A.L.E",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-01.webp",
		pixiv: "https://www.pixiv.net/en/artworks/135925950",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m25go67wxs2x",
		twitter: "https://x.com/Victim_Crasher/status/1973420126201639237",
	},
	{
		date: "2025-10-02",
		title: "Ambush",
		description: ["We heard someone got a booboo?"],
		note: ["Based on that scene from The Shining. Go watch it if you haven't, it's very good!"],
		school: "Trinity",
		characters: [
			{
				name: "Sumi Serina",
				club: "Remedial Knights",
			},
			{
				name: "Asagao Hanae",
				club: "Remedial Knights",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-02.webp",
		pixiv: "https://www.pixiv.net/en/artworks/135925950",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m27xbip5fs2o",
		twitter: "https://x.com/Victim_Crasher/status/1973783669165957526",
	},
	{
		date: "2025-10-03",
		title: "Superhero Landing",
		description: ["heard someone got a booboo? (2)"],
		note: [
			"Just like Sakurako, Mine is actually one of that students that grew on me over time.",
			"Also I really like the dynamic between Sakurako and Mine in the Trinity Idol event.",
		],
		school: "Trinity",
		characters: [
			{
				name: "Aomori Mine",
				club: "Remedial Knights",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-03.webp",
		pixiv: "https://www.pixiv.net/en/artworks/135925950",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2cip52pnc2y",
		twitter: "https://x.com/Victim_Crasher/status/1974150502595375140",
	},
	{
		date: "2025-10-04",
		title: "Gehenna Drifting",
		description: ["I heard someone got a corps- I mean, a booboo? (3)"],
		note: [
			"I might sound like a broken record again lmao, but Sena is also that character that grew on me over time.",
			'Based on the wiki, the car she\'s using is based on Horch 108 Kfz. 31 heavy off-road car. Ingame, it was named "Emergency Vehicle No. 11". I wonder what happen with the previous 10.',
		],
		school: "Gehenna",
		characters: [
			{
				name: "Himuro Sena",
				club: "Medical Emergency Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-04.webp",
		pixiv: "https://www.pixiv.net/en/artworks/135925950",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2f2xdaugs2r",
		twitter: "https://x.com/Victim_Crasher/status/1974520539927793819",
	},
	{
		date: "2025-10-05",
		title: "Santa",
		description: ["Efficient Delivery"],
		note: [
			"We moved to the Arius Squad.",
			'Based on the "From the Opera with Love!" event. Super entertaining story. The character on her sack is the boss of the gang alliance, Don Aranchino.',
		],
		school: "Arius",
		characters: [
			{
				name: "Joumae Saori",
				club: "Arius Squad",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-05.webp",
		pixiv: "https://www.pixiv.net/en/artworks/135925950",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2hd6wzbfk2m",
		twitter: "https://x.com/Victim_Crasher/status/1974845545710801269",
	},
	{
		date: "2025-10-06",
		title: "Sand Castle",
		description: ["Wishing all the best for the Arius Squad"],
		note: [
			"Shout out to Alice (@edmondbeltran on X) for the pose reference for this particular submission. Based on the Sheside Outside event.",
			"Misaki is wife, tentunya mY. Very grateful that she's much happier in her swimsuit alt.",
		],
		school: "Arius",
		characters: [
			{
				name: "Tsuchinaga Hiyori",
				club: "Arius Squad",
			},
			{
				name: "Hakari Atsuko",
				club: "Arius Squad",
			},
			{
				name: "Imashino Misaki",
				club: "Arius Squad",
			},
		],
		isLandscape: true,
		image: "/entries/2025-10-06.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136117275",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2jzl54zys2s",
		twitter: "https://x.com/Victim_Crasher/status/1975234517171659239",
	},
	{
		date: "2025-10-07",
		title: "Airi-cha~n",
		description: ["Nani ga suki?"],
		note: [
			'It\'s because Airi loves chocolate mint, the "nani ga suki" song (I forgot the title, sorry :sob:) cames to my mind immediately.',
			"I was also considering a pose based on GYARI's chocomint ice song, but in the end I decided to just go with the one you're seeing here, just because.",
		],
		school: "Trinity",
		characters: [
			{
				name: "Kurimura Airi",
				club: "After-School Sweets Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-07.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136117275",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2mjvv2hvs2i",
		twitter: "https://x.com/Victim_Crasher/status/1975596838528164259",
	},
	{
		date: "2025-10-08",
		title: "Experiment",
		description: ["This, certainly is, Rock"],
		note: ["That's a Demon Core, yep."],
		school: "Trinity",
		characters: [
			{
				name: "Ibaragi Yoshimi",
				club: "After-School Sweets Club",
			},
			{
				name: "Yutori Natsu",
				club: "After-School Sweets Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-08.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136117275",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2p3juhvrs2d",
		twitter: "https://x.com/Victim_Crasher/status/1975964785838846318",
	},
	{
		date: "2025-10-09",
		title: "Roger Trinity",
		description: ["Jika kau bertemu aku begini~"],
		note: [
			'Based on local "Roger Sumatra" meme, and the song referenced is originated from Kris - Tiara, but seen the emergence because of a cover from Raffa Affar. Go listen to the song! Both of them are good and have their own spin of the song.',
		],
		school: "Trinity",
		characters: [
			{
				name: "Kyouyama Kazusa",
				club: "After-School Sweets Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-09.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136117275",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2rl4rk2kk2q",
		twitter: "https://x.com/Victim_Crasher/status/1976323276294193532",
	},
	{
		date: "2025-10-10",
		title: "Recess",
		description: [
			"While patrolling together, Suzumi treats Reisa Ice Cream, while listening Reisa's first encounter with Kazusa back in the middle school",
		],
		note: [
			"Both us a character that I adore. Reisa is the one to protect, Suzumi is an underrated char that thankfully seen her re-emergence in her magical girl alt.",
		],
		school: "Trinity",
		characters: [
			{
				name: "Uzawa Reisa",
				club: "Trinity Vigilante Crew",
			},
			{
				name: "Morizuki Suzumi",
				club: "Trinity Vigilante Crew",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-10.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136117275",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2tqswgmx22o",
		twitter: "https://x.com/Victim_Crasher/status/1976637244304933023",
	},
	{
		date: "2025-10-11",
		title: "Exhausted",
		description: ['"Don\'t forget to take rest Ui-Senpai" - Shimiko'],
		note: [
			"The last two playable trinity character (from global), with this submission all of trinity girls should've been listed.",
			"Btw, student's halo is only visible when they're awake, so in this case when Ui is sleeping, her halo will not shown.",
		],
		school: "Trinity",
		characters: [
			{
				name: "Endo Shimiko",
				club: "Library Committee",
			},
			{
				name: "Kozeki Ui",
				club: "Library Committee",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-11.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136321510",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2wmtp4hkc2o",
		twitter: "https://x.com/Victim_Crasher/status/1977051418210271625",
	},
	{
		date: "2025-10-12",
		title: "Gastrono-Mie",
		description: ["Trying local delicacies from the faraway land"],
		note: [
			"Ada Indonesia cuy...",
			"Featuring Ind*mie.",
			" ",
			"Also Haruna's smile on her tracksuit alt is so sweet.",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Kurodate Haruna",
				club: "Gourmet Research Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-12.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136321510",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m2z3mnsmmc2w",
		twitter: "https://x.com/Victim_Crasher/status/1977405971178848481",
	},
	{
		date: "2025-10-13",
		title: "Egg",
		description: ["Junko has been eating egg for a week", "https://youtu.be/xA-4l1Weui4 (please watch it)"],
		note: [
			'Shout out to Youtube channel SipBiru and their video about "Junko Seminggu Makan Telur" (linked above). I can\'t stop hearing it.',
		],
		school: "Gehenna",
		characters: [
			{
				name: "Akashi Junko",
				club: "Gourmet Research Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-13.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136321510",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m33f7pwl2s27",
		twitter: "https://x.com/Victim_Crasher/status/1977737795956347061",
	},
	{
		date: "2025-10-14",
		title: "Water Tank",
		description: [
			'I met Akari in D.U. today, she said there is a "Bring your own tumblr" event in the nearby Angle 24 store…',
		],
		note: [
			'I know Akari is much more of an "eater" rather than "drinker", but I can\'t really put this idea out of my head lmao.',
		],
		school: "Gehenna",
		characters: [
			{
				name: "Wanibuchi Akari",
				club: "Gourmet Research Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-14.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136321510",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m362sjxy6s2z",
		twitter: "https://x.com/Victim_Crasher/status/1978123087682871593",
	},
	{
		date: "2025-10-15",
		title: "Cat Food",
		description: ['"Senseii can we please have that?? That looks good"', '"Izumi-chan, that\'s cat food…"'],
		note: [
			"The burger girl.",
			"While researching Izumi for the Inktober, I just realized she's eating a Burger with Chocolate, ooh Izumi.",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Shishidou Izumi",
				club: "Gourmet Research Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-15.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136321510",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3ajnhcros2q",
		twitter: "https://x.com/Victim_Crasher/status/1978477799666315644",
	},
	{
		date: "2025-10-16",
		title: "Pan-chan",
		description: ["The notorious pan-chan story has started once again"],
		note: ["Took a bit of a gamble here, perspective practice with almost no ref.", "(Pan-chan story pt 1/5)"],
		school: "Gehenna",
		characters: [
			{
				name: "Pan-chan",
				club: undefined,
			},
			{
				name: "Ushimaki Juri",
				club: "School Lunch Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-16.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136516446",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3cxudyuq22f",
		twitter: "https://x.com/Victim_Crasher/status/1978830335031853454",
	},
	{
		date: "2025-10-17",
		title: "Slime",
		description: ['"Fukka, don’t open the door ye-"', '"Ah…"', "And the Pan-chan saga continues…"],
		note: ["Do you notice pan-chan running in the image?", "(Pan-chan story pt 2/5)"],
		school: "Gehenna",
		characters: [
			{
				name: "Aikiyo Fuuka",
				club: "School Lunch Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-17.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136516446",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3fl2jd6ws24",
		twitter: "https://x.com/Victim_Crasher/status/1979204429535478245",
	},
	{
		date: "2025-10-18",
		title: "Pet",
		description: ['"Look Sensei, Ibuki got a new pet!"', "Well at least the pan-chan saga is over for now"],
		note: [
			"Based on short animation from BlueSechi, it's also hinted a bit that Ibuki wanted to make Pan-chan her pet, wanting to take Pan-chan for a walk and go fetch.",
			"(Pan-chan story pt 3/5)",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Tanga Ibuki",
				club: "Pandemonium Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-18.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136516446",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3i26x7ey22r",
		twitter: "https://x.com/Victim_Crasher/status/1979561517940179065",
	},
	{
		date: "2025-10-19",
		title: "Hypnotism",
		description: [
			'"So Ibuki, how did you catch your pet?"',
			'"Chiaki-senpai and Satsuki-senpai helped me"',
			"Ah… that explains it",
		],
		note: [
			"The first time I saw Satsuki I'll be like, wth mommy!? But she's apparently goofier than my initial impression.",
			"(Pan-chan story pt 4/5)",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Motomiya Chiaki",
				club: "Pandemonium Society",
			},
			{
				name: "Kyougoku Satsuki",
				club: "Pandemonium Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-19.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136516446",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3kjvp7rb22y",
		twitter: "https://x.com/Victim_Crasher/status/1979920499301081235",
	},
	{
		date: "2025-10-20",
		title: "Glory",
		description: [
			'"Iroha! Make 2000 more copies of this!"',
			'"Again? This is the third time you ask me for more copies, Makoto…"',
			'"It\'s another day in Pandemonium Society"',
		],
		note: [
			"Iroha looks so tired here lmao. I decided to not draw her hat since I'm too lazy,sorry guys...",
			"(Pan-chan story pt 5/5)",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Hanuma Makoto",
				club: "Pandemonium Society",
			},
			{
				name: "Natsume Iroha",
				club: "Pandemonium Society",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-20.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136516446",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3n37kqqzk2c",
		twitter: "https://x.com/Victim_Crasher/status/1980286410155598306",
	},
	{
		date: "2025-10-21",
		title: "Explosion",
		description: ["Definitely not OSHA compliant"],
		note: [
			"The feet girl.",
			"TBH I kinda messed up the shading here, so she looks like a lizard :sob:, sorry about that...",
			"(Hot Spring Explosion Story 1/5)",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Kinugawa Kasumi",
				club: "Hot Spring Department",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-21.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136683811",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3pkfevfac2p",
		twitter: "https://x.com/Victim_Crasher/status/1980643297224978671",
	},
	{
		date: "2025-10-22",
		title: "Dangling",
		description: ["\"That's so fun Kasumi! Let's do it again!!\"", "- Megu, while dangling on a tree"],
		note: [
			"Megu is surprisingly hard to draw, her accessories is very detailed.",
			"(Hot Spring Explosion Story 2/5)",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Shimokura Megu",
				club: "Hot Spring Department",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-22.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136683811",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3s3bzo4pc2w",
		twitter: "https://x.com/Victim_Crasher/status/1981007684838330637",
	},
	{
		date: "2025-10-23",
		title: "Spectator",
		description: [
			'"See I told you, someone\'s dangling on the tree!"',
			'"Oh my god, Kirara\'s right… are you alright miss? Want us to call the prefect team?"',
		],
		note: [
			'Erika and Kirara has a very cute dynamic. Both of them had no club affilitation initially so in the beginning of the story they were shown as "Go-Home Club" but that was changed due to series of events on "Kirara and Erika Dazzling Days" mini stories, do give it a read!',
			"(Hot Spring Explosion Story 3/5)",
		],
		school: "Gehenna",
		characters: [
			{
				name: "Hatami Erika",
				club: "Twinkle Twinkle Club",
			},
			{
				name: "Yozakura Kirara",
				club: "Twinkle Twinkle Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-23.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136683811",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3unhy2wu22m",
		twitter: "https://x.com/Victim_Crasher/status/1981377548413260002",
	},
	{
		date: "2025-10-24",
		title: "News Scoop",
		description: [
			'"We rushed immediately after we heard an explosion for the news scoop, but where the hell is Shinon? She was behind me just a moment ago"',
		],
		note: [
			"I really wish they have a dedicated story about kronos school of journalism.",
			"(Hot Spring Explosion Story 4/5)",
		],
		school: "Kronos",
		characters: [
			{
				name: "Kazemaki Mai",
				club: "Kronos News Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-24.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136683811",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3x5oh6b5k2e",
		twitter: "https://x.com/Victim_Crasher/status/1981738707179774342",
	},
	{
		date: "2025-10-25",
		title: "Blocked",
		description: ['"What do you mean I can\'t get in? This is press suppression!!"'],
		note: [
			"Shinon chara design is certainly interesting.",
			"The NPC shown is from the Prefect Team.",
			"(Hot Spring Explosion Story 5/5)",
		],
		school: "Kronos",
		characters: [
			{
				name: "Kawaru Shinon",
				club: "Kronos News Club",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-25.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136683811",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m3z454n6hk2c",
		twitter: "https://x.com/Victim_Crasher/status/1982020114531971216",
	},
	{
		date: "2025-10-26",
		title: "Rappel",
		description: ["On the way back to SCHALE, something interesting is happening on top of the Angel 24 store"],
		note: ["We started vol 4 here, but I can't really drew them all on this year's Inktober unfortunately..."],
		school: "SRT",
		characters: [
			{
				name: "Tsukiyuki Miyako",
				club: "RABBIT Squad",
			},
			{
				name: "Sorai Saki",
				club: "RABBIT Squad",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-26.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136936901",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m444zok4as2c",
		twitter: "https://x.com/Victim_Crasher/status/1982456854484853006",
	},
	{
		date: "2025-10-27",
		title: "Helicopter",
		description: [
			"After securing the leftover bentos, I saw that the Rabbit Squad's Helicopter was going uncontrollably, I wonder what happen",
		],
		note: ["I wonder what Moe and Miyu are thinking at that time lmao."],
		school: "SRT",
		characters: [
			{
				name: "Kazekura Moe",
				club: "RABBIT Squad",
			},
			{
				name: "Kasumizawa Miyu",
				club: "RABBIT Squad",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-27.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136936901",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m46nry3jfs2d",
		twitter: "https://x.com/Victim_Crasher/status/1982820386111729832",
	},
	{
		date: "2025-10-28",
		title: "Donuts",
		description: ['"Ah, my hand slipped" - Kirino'],
		note: [
			"Both of these girls has one of the hardest halo to draw, especially Fubuki lmao.",
			'Also "Nemugaki" lmao.',
		],
		school: "Valkyrie",
		characters: [
			{
				name: "Nakatsukasa Kirino",
				club: "Public Safety Bureau",
			},
			{
				name: "Nemugaki Fubuki",
				club: "Public Safety Bureau",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-28.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136936901",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m4ba4b3bjk2s",
		twitter: "https://x.com/Victim_Crasher/status/1983191064950563030",
	},
	{
		date: "2025-10-29",
		title: "This is Fine",
		description: ["Just another day in the office I guess"],
		note: ['Based on "This is Fine" meme.'],
		school: "Valkyrie",
		characters: [
			{
				name: "Ogata Kanna",
				club: "Public Peace Bureau",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-29.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136936901",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m4dmn3c4qs27",
		twitter: "https://x.com/Victim_Crasher/status/1983535748285702289",
	},
	{
		date: "2025-10-30",
		title: "Gematria",
		description: ["The antagonists of Blue Archive"],
		note: ["The most char I drew in Inktober, actually."],
		school: undefined,
		characters: [
			{
				name: "Maestro",
				club: "Gematria",
			},
			{
				name: "Golconde & Decalcomanie",
				club: "Gematria",
			},
			{
				name: "Underground Dweller",
				club: "Gematria",
			},
			{
				name: "Beatrice",
				club: "Gematria",
			},
			{
				name: "Black Suit",
				club: "Gematria",
			},
		],
		isLandscape: true,
		image: "/entries/2025-10-30.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136936901",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m4g7jpcfec26",
		twitter: "https://x.com/Victim_Crasher/status/1983909134182584641",
	},
	{
		date: "2025-10-31",
		title: "The will that always was",
		description: [
			"Thank you so much for following my journey on this years Inktober. Please let me know your favorite!",
			"62/62 Blue Archive Inktober streak",
		],
		note: [
			"When researching the characters, I notice that Phrenapates shed a single tear on their left eye. I can't imagine what horrors they had gone through...",
		],
		school: undefined,
		characters: [
			{
				name: "Phrenapates",
				club: "Iridescent Vanguard",
			},
			{
				name: "Shiroko Terror",
				club: "Iridescent Vanguard",
			},
		],
		isLandscape: false,
		image: "/entries/2025-10-31.webp",
		pixiv: "https://www.pixiv.net/en/artworks/136936901",
		blueSky: "https://bsky.app/profile/victim-crasher.com/post/3m4ippa74yk2c",
		twitter: "https://x.com/Victim_Crasher/status/1984270288700547450",
	},
];

export const schools: string[] = [...new Set(entries.map((entry) => entry.school ?? "No Affiliation"))];
export const clubsBySchool: Record<string, string[]> = Object.fromEntries(
	Object.entries(
		entries.reduce((acc, entry) => {
			const school = entry.school ?? "No Affiliation";
			if (!acc[school]) {
				acc[school] = new Set<string>();
			}
			entry.characters.forEach((character) => {
				if (character.club && character.club.includes(",")) {
					character.club.split(",").forEach((club) => {
						acc[school].add(club);
					});
				} else {
					acc[school].add(character.club ?? "No Affiliation");
				}
			});
			return acc;
		}, {} as Record<string, Set<string>>)
	).map(([school, clubs]) => [school, Array.from(clubs)])
);
export const years: string[] = [...new Set(entries.map((entry) => entry.date.split("-")[0]))];
